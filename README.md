...
Charlie alpha（模型識別碼：`Charlie-Alpha-4B`）是一個在 Apple Silicon 筆電上一晚完成的
繁中、簡中、英文數學與程式實驗模型。它以 Apache-2.0 的
...
FORGE（Focused One-pass Relative-Gap Gradient Equivalence）把一晚預算集中在高價值更新：
...
- 四組等參數短跑比較標準 LoRA、LoRA+ 與選擇性 loss。勝出配方只訓練最後 4 層 rank-32，
  正式訓練 2,896 秒，峰值記憶體 16.05 GB；最佳 validation loss 由 0.8640 降至 0.6867。
...
請求可加 `"charlie_route":"base"` 或 `"adapter"`。完整一晚流程、消融、封存與續跑入口見
