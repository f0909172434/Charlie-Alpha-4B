...
9. **单模型动态稀疏 LoRA。** 第一组 final 显示 adapter 有利于编程与中文，却损害英文数学，
   因此固定为“中文或编程启用 adapter，其余旁路”的规则。运行时只加载一份 4B 和一个
   8.52 MB adapter，切换 8 个 LoRA scale，不做第二次生成；旁路与独立底模 logits 完全一致。
...
312 条训练记录的正式训练用时 2,896 秒，峰值内存 16.05 GB；最佳 valid loss 从 0.8640
降至 0.6867。第一组 62 题冻结 final 上，直接 adapter 为 44/62，底模为 43/62（+1.62 点），
但它用英文 MATH 的退步换取了编程与中文收益。

稀疏路由在该结果之后写定，随后才建立完全不重叠的 62 题确认集。路由模型为 43/62，底模
为 42/62（+1.61 点）；编程从 11/16 到 12/16，其他语言和领域均未少答对。两组测试都多答
对 1 题，但都未达到 +2 点门槛，因此发布为 **Experimental v0.2.0**，不宣称已证明全面或
统计显著地更强。

使用 `make forge` 可运行完整且可续跑的一晚流程；各阶段入口见根目录 Makefile。完整命令
列表也收录于繁体中文版 [`FORGE.md`](FORGE.md)。
正式动态运行入口为 `make forge-router-verify`、`make forge-chat` 和 `make forge-serve`。
